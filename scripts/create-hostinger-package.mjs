...
// Create .htaccess
//
// Mirrors client/public/.htaccess so SPA routes that have a prerendered
// <route>.html file are served directly (with their own canonical, title and
// meta description), and any unknown URL falls back to the SPA shell.
...
# Strip trailing slashes with a 301 (except real directories).
...
# 1. Serve prerendered <route>.html for SPA pages when it exists, without
#    changing the visitor's URL. Generated by scripts/prerender.mjs.
RewriteCond %{REQUEST_FILENAME} !-f
RewriteCond %{REQUEST_FILENAME} !-d
RewriteCond %{DOCUMENT_ROOT}/public%{REQUEST_URI}.html -f
RewriteRule ^(.+)$ public/$1.html [L,QSA]

# 2. Handle client-side routing for SPA: any unknown URL serves the React shell.
RewriteCond %{REQUEST_FILENAME} !-f
RewriteCond %{REQUEST_FILENAME} !-d
